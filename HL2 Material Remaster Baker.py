...
def draw_hl2_panel(layout, context, compact=False):
    ...
    setup_box.prop(props, "transparent_bg")
    setup_box.prop(props, "use_gpu")

    setup_box.separator()
    setup_box.prop(props, "clear_scene_on_setup")
    setup_box.prop(props, "switch_to_shading_on_setup")
    setup_box.prop(props, "camera_view_on_setup")
    ...
    update_box.prop(props, "addon_update_json_url")
    update_box.label(text=f"Current: {get_local_addon_version_string()}")

    if props.addon_available_version:
        update_box.label(text=f"Available: {props.addon_available_version}")

    update_box.label(text=props.addon_update_status)
    ...
    if props.addon_update_changelog:
        update_box.label(text="Changelog:")

        for line in props.addon_update_changelog.splitlines()[:8]:
            update_box.label(text=line)

    if not compact:
        ref = layout.box()
        ref.label(text="Reference")
